...
Stdout is JSON: `{"name","id","pane","cwd","state":"dispatched","from","to"}`. `state=dispatched` means the brief is queued, not that it has landed. A later `[muxa] from=broker` turn means either the child never became ready (brief not pasted) or the paste was accepted but the pane still looked free (brief filed done, not retried — muxa will not re-paste a first brief). Either way, do not re-dispatch the same brief; decide what to do next yourself, e.g. `muxa tail NAME` once. Do not scrape tmux for that.
